...
```javascript
{
  ...
  role: "analyst",
  permissions: [
    "qc_queue:read", "qc_review:read", "qc_review:create",
    "qc_review:update", "qc_review:execute", "revision:create",
    "escalation:create"
  ],
  tenantId: "test-tenant",
  accessScope: {
...
TEST_JWT_ANALYST=eyJhbGciOiJIUzI1NiI...
...
curl -X POST http://localhost:3000/api/qc/validate/order-123 \
  -H "Authorization: Bearer $TEST_JWT_ANALYST"
