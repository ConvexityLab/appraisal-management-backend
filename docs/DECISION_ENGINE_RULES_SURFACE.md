...
| E | **Per-rule analytics + dashboards** (was Phase 7 of the review) — fire counts, average impact, dead-rule detection, fire-rate over time. Cross-category. | 🟡 | 2026-05-10 | — | **MVP shipped:** in-memory analytics service (no new container), `GET /:category/analytics`, Analytics tab (summary cards + sparklines + dead-rule list), `/decision-engine` landing page. **Pending follow-ups:** pre-aggregated container + daily aggregation cron when scale demands; per-rule drill-down. |
| F | **Add Review Program rules as second category** | 🟡 | 2026-05-10 | — | **Storage + workspace + analytics shipped:** category registered, FE catalog populated (report/appraiser/order vars + route_to_program / escalate / score_adjustment actions), all CRUD/audit/history works. Analytics now reads from the EXISTING `bulk-portfolio-jobs` + `review-results` containers via `ReviewProgramResultsReader` (no new container). **Pending polish:** wire MOP-side `review-program` Prio program for push/preview; replay (needs canonical variable mapping). |
...
| J | **Live-fire validation across ALL paths** | 🟡 | 2026-05-10 | — | Playwright suite shipped at `e2e/live-fire/decision-engine-suite.live-fire.spec.ts` covering 9 of the 10 matrix rows (J1, J2, J3, J4, J5, J7, J9, J10, J11). Execution pending against deployed staging — needs env flags applied + storage state captured. |
...
| L | **Axiom Criteria evaluator integration** | 🟡 | 2026-05-10 | — | L0 (survey commit at `services/decision-engine/axiom-criteria/AXIOM_INTEGRATION_SURVEY.md`) + L1 (stub analytics reader so the tab shows a "pending" panel instead of 501) shipped. L2-L5 (Axiom-side criteria-set endpoint + pusher + full reader + replay) deferred to follow-up PR since the Axiom-side endpoint doesn't exist today and is captured in the survey. |
| N | **Order Decomposition as 5th Decision Engine category** | 🟡 | 2026-05-10 | — | **N0+N1+N2 shipped:** survey commit, FE category def (status: live, appears in nav + dropdown), BE category registration with `validateRules` (DecompositionRule shape check) + stub analytics. **Pending N3-N5:** workspace CRUD controller proxying `OrderDecompositionService`, `decompositionRuleId` stamp on `VendorOrder` for real analytics, override surface. Storage stays in existing `decomposition-rules` Cosmos container — no new container. |
