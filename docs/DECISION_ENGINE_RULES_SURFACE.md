...
| C | **FE: category-aware workspace** — `/decision-engine/rules/:category`; category selector; component plugin pattern; `/auto-assignment/rules` redirects | 🟡 | 2026-05-10 | — | **MVP shipped:** new nav group, category-aware chrome (breadcrumb + dropdown + status chip), placeholder body for planned categories. **Polish pending:** factor variable catalog / action catalog / sample evaluations into per-category props on `RuleConditionBuilder` / `RulePackVisualEditor` / `RulePackPreviewPanel` so the dropdown actually swaps catalogs and not just labels. |
