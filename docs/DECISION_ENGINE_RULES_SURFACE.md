...
| I | **Cross-cutting: live feed + trace flow viz + ops dashboard** (was Phases 5/7 of the review) | 🟡 | 2026-05-10 | — | **Shipped:** polling-based `LiveDecisionsFeed` (10s refresh), `TraceFlowDiagram` React Flow visualization on order-detail trace timeline (per-trace List/Flow toggle), `OpsHealthDashboard` with cross-category 24h pips + decision counts + escalation rates + kill-switch UI surface. **Wiring still pending (separate PRs):** WebSocket/Service Bus relay so live feed isn't polling; kill-switch BE (config store + evaluator middleware that respects the flag — UI is wired, BE returns flag state as no-op today). |
