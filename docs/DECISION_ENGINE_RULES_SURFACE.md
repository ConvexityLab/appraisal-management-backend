...
| F | **Add Review Program rules as second category** | 🟡 | 2026-05-10 | — | **Storage + workspace shipped:** category registered, FE catalog populated (report/appraiser/order vars + route_to_program / escalate / score_adjustment actions), all CRUD/audit/history works through the generic surface. **Pending polish:** wire MOP-side `review-program` Prio program for push/preview/replay; add `review-program-decisions` trace store for analytics. |
| G | **Add Firing Rules as third category** | 🟡 | 2026-05-10 | — | **Storage + workspace shipped:** category registered, FE catalog populated (vendor performance vars + probation / fire / notify actions). **Pending polish:** in-process AMS evaluator that runs on a daily cron + a `firing-decisions` trace store. |
| H | **Add Axiom Criteria as fourth category** | 🟡 | 2026-05-10 | — | **Storage + raw-JSON workspace shipped:** category registered with the shared Prio validator (operators author criteria via the JSON tab). **Pending polish:** custom criteria-shaped editor (text + expected answer + rubric) replacing the visual editor for this category; Axiom integration design for push/preview/replay. |
