...
| I | **Cross-cutting: live feed + trace flow viz + ops dashboard** (was Phases 5/7 of the review) | 🟡 | 2026-05-10 | — | **Live feed MVP shipped:** polling-based `LiveDecisionsFeed` on `/decision-engine` landing (10s refresh, pulsing live indicator). **Pending polish:** WebSocket / Service Bus relay (real-time, no polling); React Flow trace visualization on order-detail; cross-tenant ops dashboard. |
