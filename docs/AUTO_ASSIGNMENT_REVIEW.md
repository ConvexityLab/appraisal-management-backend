...
| Phase 4 | **Rules Authoring Workspace (FE)** — see §13.5 | ✅ | 2026-05-09 | 2026-05-09 | T30 (RTK Query slice) + T31 (rules workspace page Active/History/Audit) + **T32 visual condition builder + raw JSON fallback** + **T33 live preview pane (MOP /preview + AMS proxy + FE pane)** + T35 (history + audit log) all shipped. 29 FE tests green (16 JSON editor parity + 13 condition builder round-trip). T34 (sandbox + replay against historical orders) deferred — needs Phase 5's traces container as substrate. T36 page-level integration tests deferred. |
| Phase 5 | **Per-Order Trace + Live Feed (visibility)** — see §13.6 | 🟡 | 2026-05-09 | — | **MVP shipped**: T37 (AssignmentTraceRecorder writes per-assignment traces to Cosmos) + T38 (GET /api/auto-assignment/traces/:orderId) + T39 (AssignmentTraceTimeline component on order-detail page with full per-vendor explanation drill-in). 9 BE recorder tests green; existing orchestrator suite unaffected. **Remaining**: T40 (Service Bus topic for assignment.evaluation.completed), T41 (WebSocket live feed page), T42+T43 (per-rule analytics aggregation + dashboard). |
