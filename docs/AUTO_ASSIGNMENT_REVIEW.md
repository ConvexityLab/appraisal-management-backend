...
| Phase 4 | **Rules Authoring Workspace (FE)** — see §13.5 | 🟡 | 2026-05-09 | — | **MVP shipped**: T30 (RTK Query slice), T31 (rules page with Active/History/Audit tabs), T32 partial (raw-JSON editor with full schema validation parity vs C++ validator), T35 (history + audit log UI). 16 validator parity tests green. Page lives at `/auto-assignment/rules`. **Remaining**: T32 polish (visual condition builder; not blocking — JSON editor is functional), T33 (live preview pane — needs MOP `/preview` endpoint), T34 (sandbox/replay), T36 (component + page-level integration tests). |
