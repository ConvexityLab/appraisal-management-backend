...
- [x] **T2.3** — Build `POST /api/criteria/evaluate` endpoint (Pattern A + B). (3 SP) `(done 2026-04-28 — new service method AxiomService.submitCriteriaReevaluation handles both patterns: A auto-resolves fileSetId from the order's most recent completed eval; B accepts caller-supplied extractedDocuments with createIfMissing:true. Posts to /api/pipelines with pipelineId: criteria-only-evaluation per QUICKSTART. Stamps _metadata on the pending eval so T1.3 enrichment + the existing webhook + fetchAndStorePipelineResults flow stores verdicts back without forking. New route: POST /api/axiom/criteria/evaluate via controller.evaluateCriteria. Updated DEFAULT_PIPELINE_IDS.CRITERIA_ONLY from stale "smart-criteria-evaluation" to canonical "criteria-only-evaluation". Errors mapped: 400 validation, 401 no tenant, 404 order-not-found, 409 NO_PRIOR_EXTRACTION (Pattern A with no prior eval), 502 upstream failure. 11/11 unit tests in axiom-criteria-evaluate.controller.test.ts. Live verification deferred — backend not running; see daily log.)`
