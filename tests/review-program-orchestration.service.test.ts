...
describe('ReviewProgramOrchestrationService.orchestrate()', () => {
  ...
  it('repairs stale seeded global programs before orchestration', async () => {
    ...
    expect(result.overallStatus).toBe('all_submitted');
    // The repaired program populates aiCriteriaRefs from the canonical data
    // in src/data/review-programs.ts, where the Axiom programId is
    // 'appraisal-qc' (renamed from 'vision-appraisal-ai').
