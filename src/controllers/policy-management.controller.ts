...
export const createPolicyManagementRouter = (dbService: CosmosDbService): Router => {
  ...
  router.put('/:id', async (req: AuthorizedRequest, res: Response): Promise<any> => {
    try {
      ...
      policyEvaluator.invalidateRuleChange(
        { tenantId: existing.tenantId, role: existing.role, resourceType: existing.resourceType },
        { tenantId: updated.tenantId, role: updated.role, resourceType: updated.resourceType },
      );
  ...
  router.delete('/:id', async (req: AuthorizedRequest, res: Response): Promise<any> => {
    try {
      ...
      policyEvaluator.invalidateCache(existing.tenantId, existing.role, existing.resourceType);
