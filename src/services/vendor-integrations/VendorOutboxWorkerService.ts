...
export class VendorOutboxWorkerService {
  private readonly logger = new Logger('VendorOutboxWorkerService');
  ...
  constructor(
    ...
    this.workerId = options?.workerId ?? `vendor-outbox-worker:${randomUUID()}`;
  }

  async start(): Promise<void> {
    if (this.isStarted) {
      this.logger.warn('VendorOutboxWorkerService already started');
      return;
    }

    this.isStarted = true;
    this.logger.info('VendorOutboxWorkerService started', {
  ...
  async stop(): Promise<void> {
    ...
    this.logger.info('VendorOutboxWorkerService stopped', { workerId: this.workerId });
  }
  ...
  private async runTick(): Promise<void> {
    ...
    try {
      ...
      this.logger.debug('Vendor outbox tick completed', {
        workerId: this.workerId,
        processedCount,
      });
    } catch (error) {
      this.logger.error('Vendor outbox tick failed', {
        workerId: this.workerId,
  ...
  private async loadPendingDocuments(nowIso: string): Promise<VendorOutboxDocument[]> {
    ...
    if (!result.success || !result.data) {
      throw new Error(result.error?.message ?? 'Failed to load pending vendor outbox documents');
    }
  ...
  private async claimDocument(
    ...
    try {
      ...
      if (errorCode === 412) {
        this.logger.debug('Vendor outbox claim lost optimistic-lock race', {
          workerId: this.workerId,
          documentId,
          tenantId,
        });
  ...
  private async failDocument(
    ...
    this.logger.warn('Vendor outbox document processing failed', {
      workerId: this.workerId,
  ...
  private toAppEvent(document: VendorOutboxDocument): AppEvent {
    const event: VendorIntegrationEvent = {
      id: document.id,
      ...
      data: {
        ...
        vendorType: document.vendorType,
        vendorOrderId: document.vendorOrderId,
        ourOrderId: document.ourOrderId,
