...
export interface WebPubSubConfig {
  /** Web PubSub endpoint URL (e.g. https://myapp.webpubsub.azure.com) */
  endpoint?: string;
  hubName?: string;
...
export class WebPubSubService {
  ...
  constructor(config: WebPubSubConfig = {}) {
    ...
    // Resolve endpoint — uses Managed Identity (DefaultAzureCredential), not connection strings
    const endpoint = config.endpoint
      || process.env.AZURE_WEB_PUBSUB_ENDPOINT
      || undefined;

    if (config.enableLocalEmulation && !endpoint) {
      // Caller explicitly asked for emulation and no real endpoint exists
      this.isEmulator = true;
    } else if (endpoint) {
      this.isEmulator = config.enableLocalEmulation === true;
    } else if (process.env.NODE_ENV === 'development' && process.env.WEB_PUBSUB_USE_EMULATOR === 'true') {
      this.isEmulator = true;
    } else {
      throw new Error('AZURE_WEB_PUBSUB_ENDPOINT environment variable is required (e.g. https://myapp.webpubsub.azure.com)');
    }

    if (this.isEmulator) {
      this.logger.info('Using local emulator mode for Web PubSub');
    } else {
      this.client = new WebPubSubServiceClient(endpoint!, new DefaultAzureCredential(), this.hubName);
      this.logger.info(`Initialized Web PubSub client for hub: ${this.hubName} (Managed Identity)`);
    }
  }
