/**
 * Seed Construction Cost Catalog
 *
 * Populates the `construction-cost-catalog` Cosmos container with ~200 representative
 * RSMeans-style unit cost items spanning all 16 CSI MasterFormat divisions.
 *
 * Costs are approximate national averages (USD, 2024 basis). They are intended for
 * feasibility review and AI budget-reasonability analysis — not for direct use on
 * bid documents.
 *
 * Run with: npx tsx src/scripts/seed-construction-catalog.ts
 *
...
import type { CostCatalogItem, CsiDivision, CostUnit } from '../types/construction-cost-catalog.types.js';

const logger = new Logger('SeedCostCatalog');
...
// ─── Site Work (often coded under Div 02 or 32 — grouped with common alt-codes) ─
const site: CostCatalogItem[] = [
...
// ─── Assemble all items ────────────────────────────────────────────────────────
const ALL_ITEMS: CostCatalogItem[] = [
...
async function main(): Promise<void> {
  logger.info(`Seeding ${ALL_ITEMS.length} cost catalog items...`);

  let upserted = 0;
  let failed = 0;

  for (const catalogItem of ALL_ITEMS) {
    try {
      await cosmosDb.upsertDocument('construction-cost-catalog', catalogItem);
      upserted++;
      if (upserted % 25 === 0) {
        logger.info(`Progress: ${upserted}/${ALL_ITEMS.length}`);
      }
    } catch (err) {
      logger.error(`Failed to upsert item ${catalogItem.id}`, { err });
      failed++;
    }
  }
