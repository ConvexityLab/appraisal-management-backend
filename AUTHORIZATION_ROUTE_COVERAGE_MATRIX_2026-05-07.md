...
| `/api/vendors` | Yes — controller-level `authorizeQuery('vendor', 'read')`, `authorizeResource('vendor', ...)`, capability gates for create/analytics, and parent `authorizeResource('order', 'update', ...)` on assignment | Yes — list/search queries append `req.authorizationFilter.sql`; detail/update/delete/availability/performance use `authorizeResource(...)`; assignment uses parent order resource auth | Complete | `src/controllers/production-vendor.controller.ts` now enforces canonical query/resource authorization across list, detail, mutation, performance, and assignment flows, with Cosmos vendor list/search queries composed through authorization filters. |
| `/api/appraisers` | Yes — controller-level `authorizeQuery('appraiser', 'read')`, `authorizeResource('appraiser', ...)`, `authorize('appraiser', 'create')`, and parent `authorizeResource('order', 'update', ...)` on assignment workflow routes | Yes — list/available queries append `req.authorizationFilter.sql`; detail/update/conflicts/licenses/pending use `authorizeResource('appraiser', ...)`; assign/accept/reject also enforce parent order auth | Complete | `src/controllers/appraiser.controller.ts`, `src/services/appraiser.service.ts`, and `src/middleware/authorization.middleware.ts` now enforce canonical query/resource authorization for the mounted appraiser routes, including assignment workflow actions. |
| `/api/documents` | Yes — controller-level `authorizeQuery('document', 'read')`, `authorizeResource('document', ...)`, capability gates for upload, and parent `authorizeResource('order', 'read', ...)` for stream resolution | Yes — list queries append `req.authorizationFilter.sql`; detail/download/version/update/delete use `authorizeResource('document', ...)`; stream resolves execution IDs or raw Axiom job IDs to a parent order before authorizing the stream | Complete | `src/controllers/document.controller.ts` now resolves `/stream/:executionId` to an `AxiomExecutionRecord`, derives `orderIdForAuth`, and enforces canonical order resource auth before proxying the SSE stream. |
...
| `/api/engagements` | Yes — controller-level `authorizeQuery('engagement', 'read')`, `authorizeResource('engagement', ...)`, and `authorize('engagement', 'create')` | Yes — list queries append `req.authorizationFilter.sql`; detail/subresource routes use `authorizeResource('engagement', ...)`; child mutation routes now anchor on the parent engagement resource | Complete | `src/controllers/engagement.controller.ts`, `src/services/engagement.service.ts`, `src/middleware/authorization.middleware.ts`, and `src/data/default-policy-rules.ts` now enforce canonical query/resource authorization for the primary engagement router. |
| `/api/engagements/:id/audit` and `/api/engagements/:id/timeline` | Yes — controller-level `authorizeResource('engagement', 'read', ...)` and `authorizeResource('engagement', 'update', ...)` on intervention | Yes — audit, timeline, event stream, and intervention routes now anchor on the parent engagement resource | Complete | `src/controllers/engagement-audit.controller.ts` now enforces canonical engagement resource authorization across audit timeline read paths, SSE event streaming, and intervention writes. |
...
| `/api/qc-workflow` | Yes — controller-level capability gates by workflow family plus parent-order `authorizeQuery('order', 'read')` / `authorizeResource('order', ...)` on queue, revision, escalation, SLA, and field-override routes | Partial parent-order scoping now enforced for list/detail/mutation flows tied to an order; aggregate-only routes still not fully canonical | Partial | `src/controllers/qc-workflow.controller.ts` now filters queue/revision/escalation list results through authorized parent orders and resolves child workflow records back to `orderIdForAuth` before mutation/detail authorization. Remaining aggregate-only routes such as `/analysts/workload`, `/queue/auto-assign`, and `/sla/metrics` are still capability-gated only. |
